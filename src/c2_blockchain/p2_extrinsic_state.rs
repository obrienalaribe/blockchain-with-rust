...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for i in 0..chain.len() {
            if self.height + 1 == chain[i].height && hash(&self) == chain[i].parent && chain[i].state == self.state + chain[i].extrinsic {
                is_valid = true;
            }
        }
        is_valid
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    let mut c1 = chain.last().unwrap().child(4);
    c1.state = 0;
    chain.push(c1);
    let c2 = c1.child(1);
    chain.push(c2);
...
fn build_forked_chain() -> (Vec<Header>, Vec<Header>) {
    let valid_chain = build_valid_chain(3);
    let forked_chain = build_an_invalid_chain();
    (valid_chain, forked_chain)
